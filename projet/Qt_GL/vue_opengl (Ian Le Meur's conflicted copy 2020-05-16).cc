...
void VueOpenGL::dessinePlateforme(QMatrix4x4 const& point_de_vue, bool en_couleur) {
    prog.setUniformValue("vue_modele", matrice_vue * point_de_vue);

    glBegin(GL_QUADS);

    if (en_couleur) {
      prog.setAttributeValue(CouleurId, 0.707, 0.3945, 0.1133); // rouge
    } else {
      prog.setAttributeValue(CouleurId, 1.0, 1.0, 1.0); // blanc
    }
...
void VueOpenGL::initializePosition()
{
  ...
  matrice_vue.rotate(30,1.0,0.0,0.0);
...
void VueOpenGL::dessineCone2(QMatrix4x4 const& point_de_vue){
    ...
    dessinePlateforme(point_de_vue);
    ...
    //if(i%2==0){
        ...
        // prog.setAttributeValue(CoordonneeTextureId, 0.5, 0.5);
         ...
         prog.setAttributeValue(CoordonneeTextureId,1+(pow((r*theta),2)/(h*h+r*r)),0.5+r);
         prog.setAttributeValue(SommetId, r*cos(theta),r*sin(theta),h);

        prog.setAttributeValue(CoordonneeTextureId,r*cos(i*theta),r*sin(i*theta));//y=h!!
        prog.setAttributeValue(SommetId, r*cos((i+1)*theta),r*sin((i+1)*theta),h);
    //}
    /*else{
        prog.setAttributeValue(CouleurId, 0.0, 1.0, 0.0);
        prog.setAttributeValue(SommetId, 0.0,h,0.0);
        prog.setAttributeValue(SommetId, r*sin(i*theta),h,r*cos(i*theta));    //y=h!!
        prog.setAttributeValue(SommetId, r*sin((i+1)*theta),h,r*cos((i+1)*theta));
    }*/
    glEnd();
    }
